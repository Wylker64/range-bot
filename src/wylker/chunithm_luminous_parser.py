...
def process_table_data(df):
    check_strings = ["MAS", "ULT", "EXP", "ORI", "撃舞", "VAR","東方","P&A","nico","イロ"]
...
def save_tables_to_excel(dfs, excel_file):
    ...
    with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
        for df, sheet_name in zip(dfs, sheet_names):
            ...
            worksheet.set_column(f'{xl_col_to_name(0)}:{xl_col_to_name(0)}', 40)
...
if html_content:
    ...
    # 保存到Excel
    save_tables_to_excel(dfs_after_removal, 'CHUNITHM_LMN_SONG_LIST.xlsx')
    print("Levels parsed to CHUNITHM_LMN_SONG_LIST.xlsx successfully.")
